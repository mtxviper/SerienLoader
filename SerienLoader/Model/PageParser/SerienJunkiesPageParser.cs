...
namespace SerienLoader.Model.PageParser
{
   public class SerienJunkiesPageParser : IPageParser
   {
      public SerienJunkiesPageParser(HtmlDocument doc)
      {
         ...
         IEnumerable<HtmlNode> postNodes = doc.DocumentNode.SelectNodes("//div").Where(x => x.GetAttributeValue("class", "") == "post").ToList();
         ...
         foreach (HtmlNode postNode in postNodes)
         {
            ...
            string format="";
            string size="";
            HtmlNode postContentNode = postNode.SelectNodes("./div").First(x => x.GetAttributeValue("class", "") == "post-content");
            ...
            foreach (HtmlNode pNode in postContentNode.ChildNodes.Where(x => x.Name == "p"))
            {
               ...
               {  
                  ...
                  {
                     throw new Exception("SeasonNumber " + seasonNumber + " found in EpisodeTitle " + seasonNumber + " differs from SeasonNumber found in SeasonTitle "+localSeasonNumber);
                  }
